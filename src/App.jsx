...
export default function App() {
  ...
  // === Tema & Tipografías (dinámico, persistente) ===
  ...
  const monthName = new Date(current.y, current.m, 1)
  ...
  return (
    <div className="min-h-screen bg-app text-app font-body">
      {/* NAV */}
      <header className="sticky top-0 z-50 bg-card/80 backdrop-blur border-b border-app">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <a href="#home" className="font-display font-black tracking-tight text-xl">HBF</a>

          <nav className="hidden md:flex gap-6 text-sm">
            <a href="#grilla" className="hover:text-muted flex items-center gap-2"><CalendarDays className="h-4 w-4" />Grilla</a>
            <a href="#galeria" className="hover:text-muted flex items-center gap-2"><Images className="h-4 w-4" />Fotos</a>
            <a href="#proyecto" className="hover:text-muted flex items-center gap-2"><Info className="h-4 w-4" />Proyecto</a>
            <a href="#publicidades" className="hover:text-muted flex items-center gap-2"><Megaphone className="h-4 w-4" />Publicidades</a>
            <a href="#contacto" className="hover:text-muted flex items-center gap-2"><GalleryHorizontalEnd className="h-4 w-4" />Contacto</a>
          </nav>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Botón entradas con color de marca */}
            <a
      ...
      <section id="home" className="relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-b from-amber-100/40 via-transparent to-transparent pointer-events-none" />
        <div className="max-w-6xl mx-auto px-4 py-12 md:py-20 grid md:grid-cols-2 gap-10 items-center">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <p className="mt-4 text-lg text-muted">{BRAND.slogan}</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="#grilla" className="inline-flex items-center px-3 py-2 rounded-2xl btn-brand text-sm"><CalendarDays className="h-4 w-4 mr-1" />Ver grilla</a>
              <a href="#galeria" className="inline-flex items-center px-3 py-2 rounded-2xl btn-ghost text-sm"><Images className="h-4 w-4 mr-1" />Ver fotos</a>
              <a href={BRAND.entradasLink} target="_blank" rel="noreferrer" className="inline-flex items-center px-3 py-2 rounded-2xl btn-ghost text-sm"><Ticket className="h-4 w-4 mr-1" />Comprar entradas</a>
            </div>
            <div className="mt-6 text-sm text-muted flex flex-col gap-1">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.1 }} className="relative">
            <div className="aspect-[4/3] rounded-3xl overflow-hidden shadow-xl bg-card">
              <img src="/img/bar-herederos-socios.png" alt="Socios" className="w-full h-full object-cover"/>
            ...
            <div className="absolute -bottom-5 -left-5 bg-card rounded-2xl shadow p-4 flex items-center gap-3 border border-app">
              ...
              <div>
                ...
                <p className="text-xs text-muted">Todos los fines de semana</p>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {/* GRILLA DEL MES */}
      <section id="grilla" className="py-14 bg-card border-t border-app">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-display text-2xl md:text-3xl font-extrabold">Grilla del mes</h2>
            <div className="flex items-center gap-2">
              <button onClick={prev} className="p-2 rounded-full hover:bg-card border border-app"><ChevronLeft className="h-5 w-5" /></button>
              <div className="px-3 py-1 rounded-full bg-card border border-app text-sm font-medium capitalize">{monthName}</div>
              <button onClick={next} className="p-2 rounded-full hover:bg-card border border-app"><ChevronRight className="h-5 w-5" /></button>
            </div>
          </div>

          <div className="grid grid-cols-7 gap-px bg-card rounded-xl overflow-hidden text-xs md:text-sm border border-app">
            {["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"].map((d) => (
              <div key={d} className="bg-card font-semibold text-center py-2 border-b border-app">{d}</div>
            ))}
            {matrix.map((week) =>
              week.map((d) => {
                ...
                return (
                  <div
                    key={d.key}
                    className={`min-h-[110px] p-2 ${d.inMonth ? "bg-card" : "bg-card text-muted"} border-b border-r border-app`}
                  >
                    <div className="text-[11px] md:text-xs font-semibold opacity-70">{d.date.getDate()}</div>
                    <div className="mt-1 flex flex-col gap-1">
                      {evs.map((e, i) => (
                        <a href={BRAND.entradasLink} key={i} className="block rounded-lg border border-app hover:shadow-sm transition p-2 bg-card">
                          ...
                          <div className="mt-1 flex flex-wrap gap-1">
                            {e.etiquetas?.map((t) => (
                              <span key={t} className="text-[9px] px-1.5 py-0.5 rounded-full border border-app bg-card">#{t}</span>
                            ))}
                          </div>
                        </a>
                      ))}
          ...
          <div className="mt-8 grid md:grid-cols-3 gap-4">
            {EVENTOS.slice(0, 3).map((e, idx) => (
              <div key={idx} className="rounded-2xl border border-app bg-card">
                <div className="p-4 border-b border-app">
                  <div className="text-base font-semibold flex items-start justify-between gap-3">
                    ...
                    <span className="text-sm font-normal text-muted">
                      {new Date(e.date).toLocaleDateString("es-AR", { day: "2-digit", month: "short" })}
                    </span>
                  </div>
                </div>
                <div className="p-4 text-sm">
                  ...
                  <a href={BRAND.entradasLink} className="mt-3 w-full inline-flex justify-center items-center gap-2 px-3 py-2 rounded-xl btn-brand text-sm">
                    <Ticket className="h-4 w-4" />Sacar entrada
                  </a>
                </div>
              </div>
            ))}
      ...
      {/* GALERÍA */}
      <section id="galeria" className="py-14 bg-card">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="font-display text-2xl md:text-3xl font-extrabold mb-6">Fotos</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            {GALERIA.map((g, i) => (
              <motion.div
                ...
                className="aspect-square rounded-2xl overflow-hidden border border-app bg-card"
              >
      ...
      {/* PROYECTO / INFO */}
      <section id="proyecto" className="py-14 bg-card border-y border-app">
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <div className="mt-6 flex gap-3">
              <a href={BRAND.entradasLink} className="inline-flex items-center px-3 py-2 rounded-2xl btn-brand text-sm"><Ticket className="h-4 w-4 mr-1" />Comprar entradas</a>
              <a href="#contacto" className="inline-flex items-center px-3 py-2 rounded-2xl btn-ghost text-sm"><ExternalLink className="h-4 w-4 mr-1" />Ser sponsor</a>
            </div>
          </div>
          <div>
            <div className="rounded-2xl border border-app bg-card">
              ...
              <div className="p-4 text-sm grid grid-cols-2 gap-y-2">
                <div>
      ...
      <section id="publicidades" className="py-14 bg-card">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="font-display text-2xl md:text-3xl font-extrabold mb-6">Publicidades</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {SPONSORS.map((s, i) => (
              <a key={i} href={s.url} className="bg-card border border-app rounded-2xl p-4 flex items-center justify-center hover:shadow-sm">
                <img src={s.logo} alt={s.nombre} className="h-14 object-contain" />
              </a>
            ))}
          </div>
          <p className="mt-4 text-sm text-muted">¿Querés ser parte? Escribinos y te enviamos el media kit.</p>
        </div>
      </section>
    ...
    <div className="rounded-2xl border bg-white">
      ...
      <footer className="py-8 bg-card border-t border-app">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="text-xs text-muted">© {new Date().getFullYear()} – Hecho con ❤ en Olavarría</div>
        </div>
      </footer>
    </div>
  );
